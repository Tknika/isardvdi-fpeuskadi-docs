            ...
            if ($CFG->branch >= 37) {
                ...
                if (stripos($text, '{course_field_') !== false) {
                    ...
                    foreach ($coursefields as $field => $value) {
                        ...
                        //>>MUA
                        if (stripos($text, '{course_field_vdi_estekak}') !== false) {
                                preg_match_all('/href\=\"(.*)\"/', $value, $matches);
                                $urlak = $matches[1]; //$value aldagitik href estekak lortu
                                if (!isset($balioa))
                                        $balioa = '';
                                $i = 0;
                                foreach ($urlak as $file_url) {
                                        //print_r($file_url);
                                        $file_url = str_replace("https", "http", $file_url); //ikastaroak.birt.eus-ek 443 conection refused
                                        ...
                                        curl_close($ch);
                                        $lerroak = explode("\n", $content);
                                        foreach ($lerroak as $lerroa) { //CSV-ko lerro bakoitzeko
                                                $lerro_datuak = explode(',', $lerroa);
                                                if (sizeof($lerro_datuak) == 4){ //Lerro zuriak ekidin
                                                        $username = explode('@', $lerro_datuak[2]);
                                                        if ($username[0] == $USER->username) {
                                                                $i++;
                                                                $balioa .= $i.') <a href='.$lerro_datuak[3].' target="_blank" style="color:blue">'.$lerro_datuak[3].'</a><br>';
                                                        }
                                                }
                                        }
                                }
                                $replace['/\{course_field_vdi_estekak\}/i'] = format_text($balioa, FORMAT_HTML, ['context' => $coursecontext]);
                        }
                        //<<MUA
                    }
                }
